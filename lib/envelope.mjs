...
export function makeEnvelope({ from, to, prompt, trace_id, parent_id, hop = 0, max_hops = DEFAULT_MAX_HOPS, deadline_ms = DEFAULT_DEADLINE_MS, budget_tokens }) {
  ...
  return { v: V, trace_id: trace_id || newTraceId(), parent_id: parent_id || null, from, to, hop, max_hops, deadline_ms, budget_tokens: budget_tokens ?? null, prompt };
}
